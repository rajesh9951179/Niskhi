...
function Createaccount() {
  ...
  const validateFirstName = (firstName) => {
    if (!firstName) return 'First Name is required.';
    if (/[^a-zA-Z]/.test(firstName)) return 'Invalid format. Only letters allowed.';
    return '';
  };

  const validateLastName = (lastName) => {
    if (!lastName) return 'Last Name is required.';
    if (/[^a-zA-Z]/.test(lastName)) return 'Invalid format. Only letters allowed.';
    return '';
  };
  ...
  const validatePassword = (password) => {
    if (!password) return 'Password is required.';
    if (!/^(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/.test(password)) {
      return 'Password must be 8+ characters, include an uppercase letter, a number, and a special character.';
    }
    return '';
  };
  ...
  return (
    <div style={{ 
      ...
      <form 
        ...
        <h2 style={{ textAlign: 'center', marginBottom: '20px', fontSize: '1.7rem' }}>Create Account</h2>
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          <input 
            ...
            style={{ height: '50px', width: '100%' }} 
          />
          {errors.pswd && <p style={{ color: 'red' }}>{errors.pswd}</p>}
        </div>
